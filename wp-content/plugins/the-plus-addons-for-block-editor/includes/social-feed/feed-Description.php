<?php
    ...
    if(!empty($txtLimt)){
        ...
        // First text
            ...
            $firstdesc = preg_replace($url, '<a href="$0" target="_blank" rel="noopener noreferrer" class="tpgb-feedurl" title="" >$0</a>', $HashTag);
        // Total text
            $Mantion = preg_replace('/(^|\s)@([\w.]+)/', '$1<span class="tpgb-mantion" >@$2</span>', $totaldesc);
            $HashTag = preg_replace("/#(\\S+)/", '<span class="tpgb-hashtag" >$0 </span>', $Mantion);
            $totaldesc = preg_replace($url, '<a href="$0" target="_blank" rel="noopener noreferrer" class="tpgb-feedurl" title="" >$0</a>', $HashTag);
        ?> 
            <div class="tpgb-message">
                <div class="showtext"><?php echo wp_kses_post($firstdesc); ?>
                    <?php if(($TextType == 'char' && ($ltn > strlen($firstdesc))) || ($TextType == 'word' && ($ltn > count(explode(" ", $firstdesc))) )){ ?>
                        ...
                        <a class="readbtn"><?php echo wp_kses_post($TextMore); ?> </a>
                    <?php } ?>
                </div>
            </div>
        <?php
    }else{
        $feedurl = preg_replace($url, '<a href="$0" target="_blank" rel="noopener noreferrer" class="tpgb-feedurl" title="" >$0</a>', $Description);
