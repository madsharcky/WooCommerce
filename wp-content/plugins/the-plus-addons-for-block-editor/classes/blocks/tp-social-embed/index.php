	...
	$output .= '<div class="tpgb-block-'.esc_attr($block_id).' tpgb-social-embed '.esc_attr($blockClass).'">';
		...
		if( $embedType == 'facebook' ){
			...
			if( $type == 'posts' ){
				...
				$output .= '<iframe class="tpgb-fb-iframe" src="https://www.facebook.com/plugins/post.php?href='.esc_url($postURL).'&show_text='.esc_attr($attributes['fullPT']).'&width='.esc_attr($wdPost).'&height='.esc_attr($hgPost).'&appId=" width="'.esc_attr($wdPost).'" height="'.esc_attr($hgPost).'" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media" ></iframe>';
			}
			if( $type == 'videos' ){
			...
			if( $type == 'page' ){
				...
				$output .= '<iframe class="tpgb-fb-iframe" src="https://www.facebook.com/plugins/page.php?href='.esc_url($uRLP).'&tabs='.esc_attr($attributes['layoutP']).'&width='.esc_attr($wdPage).'&height='.esc_attr($hgPage).'&small_header='.esc_attr($attributes['smallHP']).'&hide_cover='.esc_attr($attributes['coverP']).'&show_facepile='.esc_attr($attributes['profileP']).'&hide_cta='.esc_attr($attributes['ctaBtn']).'&lazy=true&adapt_container_width=true&appId=" width="'.esc_attr($wdPage).'" height="'.esc_attr($hgPage).'" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media" ></iframe>';
			}
			if( $type == 'save' ){
			...
			if( $type == 'share' ){
				...
				$output .= '<iframe src="https://www.facebook.com/plugins/share_button.php?href='.esc_url($shareURL).'&layout='.esc_attr($attributes['shareBtn']).'&size='.esc_attr($sizeBtn).'&width='.esc_attr($shareW).'&height='.esc_attr($shareH).'&appId=" width="'.esc_attr($shareW).'" height="'.esc_attr($shareH).'" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media"></iframe>';
			}
			...
			$output .= '<iframe width="'.esc_attr($exWidth).'" height="'.esc_attr($exHeight).'" src='.esc_attr($ytSrc).' frameborder="0" allow="accelerometer; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
			...
			if($mapaccesstoken == 'default'){
				$output .= '<iframe class="tpgb-gmap-embed" src="http://maps.google.com/maps?q='.esc_attr($gSearchText).'&z='.esc_attr($mapZoom).'&output=embed" height="'.esc_attr($gMHeight).'" loading="lazy" allowfullscreen frameborder="0" scrolling="no"></iframe>';
				...
				if(!empty($gAccesstoken)){
					...
					if($gMapModes == "place"){
						$output .= '<iframe class="tpgb-gmap-embed" src="https://www.google.com/maps/embed/v1/place?key='.esc_attr($gAccesstoken).'&q='.esc_attr($gSearchText).'&zoom='.esc_attr($mapZoom).'&maptype='.esc_attr($mapViews).'&language=En" height="'.esc_attr($gMHeight).'" loading="lazy" allowfullscreen></iframe>';
						...
						$output .= '<iframe class="tpgb-gmap-embed" src="https://www.google.com/maps/embed/v1/directions?key='.esc_attr($gAccesstoken).esc_attr($gOrigin).esc_attr($gDestination).esc_attr($gWaypoints).esc_attr($Gavoid).'&mode='.esc_attr($gTravelMode).'&zoom='.esc_attr($mapZoom).'&maptype='.esc_attr($mapViews).'&language=En" height="'.esc_attr($gMHeight).'" loading="lazy" allowfullscreen ></iframe>';
					}else if($gMapModes == "streetview"){
						$gstreetviewText = (!empty($attributes['gstreetviewText'])) ? $attributes['gstreetviewText'] : '';

						$output .= '<iframe class="tpgb-gmap-embed" src="https://www.google.com/maps/embed/v1/streetview?key='.esc_attr($gAccesstoken).'&location='.esc_attr($gstreetviewText).'&heading=210&pitch=10&fov=90" height="'.esc_attr($gMHeight).'" loading="lazy" allowfullscreen></iframe>';
					}else if($gMapModes == "search"){
						$output .= '<iframe class="tpgb-gmap-embed" src="https://www.google.com/maps/embed/v1/search?key='.esc_attr($gAccesstoken).'&q='.esc_attr($gSearchText).'&zoom='.esc_attr($mapZoom).'&maptype='.esc_attr($mapViews).'&language=En" height="'.esc_attr($gMHeight).'" loading="lazy" allowfullscreen ></iframe>';
					}
				}else{
