...
function tpgb_tp_search_bar_render_callback( $attr, $content) {
	...
	$output .= '<div class="tpgb-search-bar tpgb-block-'.esc_attr($block_id).' '.esc_attr($blockClass).' " data-id="'.esc_attr($block_id).'" data-ajax_search= \'' .$dataattr. '\' data-result-setting= \''.$ResultOnOff.'\' data-genericfilter='.$GFarray.' data-pagination-data= \''.$PageJson.'\' data-acfdata='.esc_attr($AcfData).' data-default-data= \''.$DefaultSetting.'\'>';
				...
				$output .= '<div class="tpgb-input-field">';
					...
					$output .= '<div class="tpgb-input-label-field">';
						if(!empty($searchLabel)){
							$output .= '<label class="tpgb-search-label">'.esc_html__( $searchLabel, 'tpgb' ).'</label>';
						}
					$output .= '</div>';
						...
						$output .= '<div class="tpgb-btn-wrap">';
							$output .= '<button class="tpgb-search-btn" name="submit" >';
								...
								if(!empty($searchBtn['sBtnText'])){
									$output .= '<span class="tpgb-search-btn-txt '.esc_attr($searchBtn['sIconPos']).'">'.esc_html__($searchBtn['sBtnText']).'</span>';
								}
		...
		$output .= '<div class="tpgb-search-area '.esc_attr($resultStyle).'">';
			...
			$output .= '<div class="tpgb-search-header">';
...
function tpgb_search_drop_down($data, $name, $id, $taxo, $repeater){
	...
	if(!empty($label)){
		$select .= '<label class="tpgb-search-label">'.esc_html__( $label , 'tpgb' ).'</label>';
	}
	...
	$select .= '<div class="tpgb-sbar-dropdown">';
		$select .= '<div class="tpgb-select">';
			$select .= '<span class="search-selected-text">'.esc_html__($placeH).'</span><span class="tpgb-dd-icon"><i class="fas fa-chevron-down"></i></span>';
		$select .= '</div>';
		$select .= '<input type="hidden" name="'.esc_attr($DatName).'" id="'.esc_attr($DatName).'" >';
		$select .= '<ul class="tpgb-sbar-dropdown-menu">';
			$select .= '<li id="" class="tpgb-searchbar-li">'.esc_html__($placeH).'</li>';
			foreach($data as $key => $label){
				...
				$select .= '<li id="'.$key.'" class="tpgb-searchbar-li" >';
					if($showCnt=='yes') {
						$select .= esc_html__($LName) .' ('.esc_html__($Lcount).')';
					}else {
						$select .= esc_html__($LName);
					}
