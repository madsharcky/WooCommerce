...
class Tp_Core_Init_Blocks {
	...
	public function enqueue_load_block_css_js( $check_load = false ){
		...
		$GoogleMap_Enable = Tp_Blocks_Helper::get_extra_option('gmap_api_switch');
		$GoogleMap_Api = '';
		if(!empty($GoogleMap_Enable) && (($GoogleMap_Enable=='enable' && has_block( 'tpgb/tp-google-map' )) || ($GoogleMap_Enable=='enable' && class_exists( 'GeneratePress_Elements_Helper' )))){
			$GoogleMap_Api = Tp_Blocks_Helper::get_extra_option('googlemap_api');
		...
		$this->tpgb_reusable_block_css($this->is_tpgb_post_id());
		if( isset($_GET['preview']) && $_GET['preview'] == true && file_exists($preview_css_path)){
			...
			if (file_exists($preview_css_path)) {
				...
				if (!$this->is_editor_screen()) {
					wp_enqueue_style("plus-preview-{$post_id}", tpgb_library()->pathurl_security($css_url), false, $plus_version);
				}
			}
		...
		if(!empty($get_opt) && isset($get_opt[$queried_obj]) && !empty($get_opt[$queried_obj]) ){
			foreach($get_opt[$queried_obj] as $template_id){
				...
				if (file_exists($css_path)) {
					...
					if (!$this->is_editor_screen()) {
						wp_enqueue_style("plus-post-{$template_id}", tpgb_library()->pathurl_security($css_url), array('tpgb-plus-block-front-css'), $plus_version);
					}
				}
			}
		}
		...
		if(!class_exists('Kadence_Pro') && !class_exists('Kadence_Pro\Elements_Post_Type_Controller')){
			return;
		}
