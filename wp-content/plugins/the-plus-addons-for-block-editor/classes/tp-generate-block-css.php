...
class Tp_Generate_Blocks_Css {
								...
								foreach($attr_value['style'] as $indexStyle => $selectData) {
									...
									if ($this->conditions_styling($block_value, $selectData, $attr_key, $indexStyle)) {
										...
										if ( isset( $block_value[$attr_key]['value'] ) && (gettype( $block_value[$attr_key]['value'] ) == 'array' || gettype( $block_value[$attr_key]['value'] ) == 'object') ) {
														...
														if($matches){
															if( !empty($output_array[1]) ){
																foreach( $output_array[1] as $newKey ){
																	if(gettype($block_value[$newKey]['value']['md']) == 'object' || gettype($block_value[$newKey]['value']['md']) == 'array'){
												...
												if($dimension!=''){
													...
													if(isset($SelectorData[0]) && strpos($SelectorData[0], '{{') ){
														...
														if($matches){
															if( !empty($output_array[1]) ){
																foreach( $output_array[1] as $newKey ){
																	if(gettype($block_value[$newKey]['value']['sm']) == 'object' || gettype($block_value[$newKey]['value']['sm']) == 'array'){
												...
												if( $dimension!='' ){
													...
													if(isset($SelectorData[0]) && strpos($SelectorData[0], '{{') ){
														...
														if($matches){
															if( !empty($output_array[1]) ){
																foreach( $output_array[1] as $newKey ){
																	if(gettype($block_value[$newKey]['value']['xs']) == 'object' || gettype($block_value[$newKey]['value']['xs']) == 'array'){
											...
											if ($attr_key == 'hideTablet') {
												if ($isInline) {
													$sm = array_merge($sm, $this->singleField($cssSelecor, $blockID, $attr_key, $block_value[$attr_key]['value'], 'tpgb'));
												}
											} else if ($attr_key == 'hideMobile') {
												if ($isInline) {
													$xs = array_merge($xs, $this->singleField($cssSelecor, $blockID, $attr_key, $block_value[$attr_key]['value'], 'tpgb'));
												}
											} else {
												...
												if ($this->conditions_styling($itemData, $selectData, $attrKey, $indexStyle)) {
													...
													if ( isset( $itemData[$attrKey]['value'] ) && (gettype( $itemData[$attrKey]['value'] ) == 'array' || gettype( $itemData[$attrKey]['value'] ) == 'object') ) {
														...
														if ($itemData[$attrKey]['value']) {
															...
															if(strpos($objectCss[0], '{{')){
																$matches = preg_match_all('/\{{(.*?)\}}/', $objectCss[0], $output_array);
																if($matches){
																	if( !empty($output_array[1]) ){
																		foreach( $output_array[1] as $newKey ){
																			if(gettype($block_value[$newKey]['value']) == 'object' || gettype($block_value[$newKey]['value']) == 'array'){
						...
						if (isset($objData['relation']) && $objData['relation'] == '==' ) {
							...
							if (isset($objData['value']) && (gettype($objData['value']) == 'string' || gettype($objData['value']) == 'number' || gettype($objData['value']) == 'boolean')) {
								...
								if ( $settings[$objData['key']]['value'] === $objData['value']) {
									...
									if( ($settings[$objData['key']]['value']['md'] == $objData['value']) || (isset($settings[$objData['key']]['value']['sm']) && $settings[$objData['key']]['value']['sm'] == $objData['value']) || (isset($settings[$objData['key']]['value']['xs']) && $settings[$objData['key']]['value']['xs'] == $objData['value']) ){
										$select = true;
									}
	...
	public function _device( $val, $selector ){
		...
		if ($val && isset($val['md']) && $val['md']!='') {
			$data['md'] =  str_replace('{{key}}', $val['md'] . (!empty($val['unit']) ? $val['unit'] : ''), $selector);
		}
		if ($val && isset($val['sm']) && $val['sm']!='') {
			$data['sm'] = str_replace('{{key}}', $val['sm'] . (!empty($val['unit']) ? $val['unit'] : ''), $selector);
		}
		if ($val && isset($val['xs']) && $val['xs']!='') {
			$data['xs'] = str_replace('{{key}}', $val['xs'] . (!empty($val['unit']) ? $val['unit'] : ''), $selector);
		}
		return $data;
	}
	/*
