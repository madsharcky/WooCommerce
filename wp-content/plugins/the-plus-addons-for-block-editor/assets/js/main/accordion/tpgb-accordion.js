window.addEventListener('DOMContentLoaded', (event) => {
  let allAccordion = document.querySelectorAll('.tpgb-accordion-wrapper');
  if(allAccordion){
        allAccordion.forEach((el)=>{
          let accType = el.getAttribute('data-type'),
              accrodionList = el.querySelectorAll('.tpgb-accordion-item');
      ...
      window.addEventListener("DOMContentLoaded", () =>{
        if(hash!='' && hash!=undefined){
          ...
          if(!getHash.classList.contains('active') && getHash){
            ...
            let mainAc = getHash.closest('.tpgb-accordion-wrapper'),
                acList = mainAc.querySelectorAll('.tpgb-accordion-item');
